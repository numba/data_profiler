/*
 * This code is based on the _lsprof.c file in the Python source tree.
 * See the file LICENSE in the Python source distribution for its copyright.
 */
...
#define GET_CFC(result, self)						\
  if (((ProfilerObject *)self)->flags & POF_SIGNATURES)			\
  {									\
    result = PyObject_GetAttrString(self, "_create_function_call");	\
    if (!result)							\
    {									\
      PyErr_SetString(PyExc_RuntimeError,				\
		      "missing Profiler._create_function_call attribute"); \
      return -1;							\
    }									\
  }									\
                  ...
                  PyObject *arg)
{
...
#ifdef FUNCTION_CALL
    {
        PyObject *create_function_call = NULL;
	void *key = 0;
        PyObject *fcall = 0;
	GET_CFC(create_function_call, self);
        if (create_function_call)
        {
	  ProfilerEntry *profEntry;
    	  fcall = PyObject_CallFunction(create_function_call, "O", frame);
 	  Py_DECREF(create_function_call);
	  if (!fcall) return -1;
	  // check whether we already have it:
	  key = (void*)PyObject_Hash(fcall);
          profEntry = getEntry((ProfilerObject *)self, key);
	  if (profEntry != NULL)
	  {
	    // use the original one
	    Py_DECREF(fcall);
	    fcall = profEntry->userObj;
	  }
	}
        else
	{
	  key = (void *)frame->f_code;
	  fcall = (PyObject *)frame->f_code;
	}
        ptrace_enter_call(self, key, fcall);
        break;
    }
#else
	...
	GET_CFC(create_function_call, self);
        ...
        {
	  ProfilerEntry *profEntry;
	  PyObject *name = normalizeUserObj(arg);
    	  fcall = PyObject_CallFunction(create_function_call, "OO", frame, name);
 	  Py_DECREF(create_function_call);
	  if (!fcall) return -1;
	  // check whether we already have it:
	  key = (void*)PyObject_Hash(fcall);
	...
	{
	  key = (void *)((PyCFunctionObject *)arg)->m_ml;
	  fcall = arg;
	}
        ptrace_enter_call(self, key, fcall);
#else
...
static int
setSignatures(ProfilerObject *pObj, int nvalue)
{
    if (nvalue == 0)
        pObj->flags &= ~POF_SIGNATURES;
    else if (nvalue > 0)
        pObj->flags |=  POF_SIGNATURES;
    return 0;
}
...
profiler_enable(ProfilerObject *self, PyObject *args, PyObject *kwds)
{
    ...
    int builtins = -1;
    int signatures = -1;
    static char *kwlist[] = {"subcalls", "builtins", "signatures", 0};
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|iii:enable",
                                     kwlist, &subcalls, &builtins, &signatures))
        return NULL;
    if (setSubcalls(self, subcalls) < 0 ||
	setBuiltins(self, builtins) < 0 ||
	setSignatures(self, signatures) < 0)
        return NULL;
...
profiler_init(ProfilerObject *pObj, PyObject *args, PyObject *kw)
{
    ...
    static char *kwlist[] = {"timer", "timeunit",
			     "subcalls", "builtins", "signatures", 0};

    if (!PyArg_ParseTupleAndKeywords(args, kw, "|Odiii:Profiler", kwlist,
                                     &timer, &timeunit,
                                     &subcalls, &builtins, &signatures))
        return -1;

    if (setSubcalls(pObj, subcalls) < 0 ||
	setBuiltins(pObj, builtins) < 0 ||
	setSignatures(pObj, signatures) < 0)
        return -1;
...
{
    PyObject *module, *d;
    module = Py_InitModule3("prof", moduleMethods, "Fast profiler");
    if (module == NULL)
        return;
